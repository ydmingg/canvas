...
var handler = function (curpath, search, hash, rutes) {
    var idx = rutes.lists.indexOf(curpath);
    if (idx === -1) {
        var index = routeHandler(curpath, rutes);
        if (index === -1) {
            return;
        }
        idx = index;
    }
    var urlMatch = match(rutes.lists[idx], {
        decode: decodeURIComponent,
    });
    var ruteinfo = urlMatch(curpath);
    rutes.query = {};
    rutes.query = ruteinfo === null || ruteinfo === void 0 ? void 0 : ruteinfo.query;
    searchhandler(search, rutes);
    rutes.callbacks[idx](rutes);
};
var routeHandler = function (pathname, rutes) {
    var routeList = rutes.lists;
    for (var i = 0; i < routeList.length; i++) {
        if (!routeList[i].includes('*'))
            continue;
        if (!pathname.includes(routeList[i].split('*')[0]))
            continue;
        return i;
    }
    return -1;
};
var searchhandler = function (search, rutes) {
    var obj = {};
...
var Context$2 = (function () {
    function Context(href, patter) {
        var t = this;
        t.patter = patter;
        t.href = href;
        t._toURL(href);
    }
    Context.prototype._toURL = function (href) {
        var t = this;
        var urlinfo = new URL(href, window.location.toString());
        t.href = href;
        t.protocol = urlinfo.protocol;
        t.hostname = urlinfo.hostname;
        t.port = Number(urlinfo.port);
        t.pathname = urlinfo.pathname;
        t.search = urlinfo.search;
        t.hash = urlinfo.hash;
    };
    Context.prototype._sameOrigin = function () {
        var t = this;
        var loc = window.location;
        return t.protocol === loc.protocol &&
            t.hostname === loc.hostname &&
            (t.port === Number(loc.port) && (Number(loc.port) === t.port || loc.port === '' && t.port == 80 || t.port == 443));
    };
    Context.prototype._samePath = function () {
        var t = this;
        var loc = window.location;
        return t.pathname === loc.pathname &&
            t.search === loc.search && t.hash == loc.hash;
    };
    Context.prototype._savehistory = function () {
        var t = this;
        window.history.pushState({}, "", t.pathname + t.search);
    };
    Context.prototype.handler = function (rutes) {
        var t = this;
        if (!t._sameOrigin()) {
            t.redirect_out();
        }
        ...
        ruting.handler(t.pathname, t.search, t.hash, rutes);
    };
    Context.prototype.redirect_out = function () {
        var t = this;
...
var _which = function (evt) {
    evt = evt || (hasWindow && window.event);
    return null == evt.which ? evt.button : evt.which;
};
var _checkIsClick = function (evt) {
...
var _matchHref = function (evt) {
    var el = evt.target;
    var eventPath = evt.path || (evt.composedPath ? evt.composedPath() : null);
    if (eventPath) {
...
var clickchange = function (evt, rutes) {
    evt.preventDefault();
    if (1 !== _which(evt))
        return;
    if (!_checkIsClick)
        return;
    var route_href = _matchHref(evt);
    if (route_href == "" || route_href == undefined)
        return;
    var ctx = new Context$2(route_href, 'A');
    ctx.handler(rutes);
};
var hashchange = function (evt, rutes) {
    var href = evt.target.URL;
    var ctx = new Context$2(href, "H");
    ctx.handler(rutes);
};
var popstatechange = function (evt, rutes) {
    var href = evt.target.location.href;
    rutes.params = evt.state;
    var ctx = new Context$2(href, "R");
    ctx.handler(rutes);
};
var historyStateChange = function (evt, rutes) {
    var href = evt.target.location.href;
    rutes.params = evt.stateInfo[0];
    var ctx = new Context$2(href, "R");
    ctx.handler(rutes);
};
var loadchange = function (evt, rutes) {
    var href = evt.target.URL;
    var ctx = new Context$2(href, "L");
    ctx.handler(rutes);
};
var control = {
    clickchange: clickchange,
    hashchange: hashchange,
    popstatechange: popstatechange,
    loadchange: loadchange,
    historyStateChange: historyStateChange
};

var rutelist = (function () {
    function rutelist() {
        this.len = 0;
        var t = this;
        t.len = 0;
        t.lists = [""];
        t.callbacks = [function () { }];
    }
    rutelist.prototype.reg = function (path, fn) {
        var t = this;
        if (t.is_reg(path)) {
            console.log("路由重复");
            return;
    ...
    rutelist.prototype.is_reg = function (path) {
        var t = this;
    ...
    rutelist.prototype.check_clash = function (path) {
    };
    return rutelist;
}());

var addStateListener = function () {
    var listener = function (type) {
        var orig = history[type];
        return function () {
            var rv = orig.apply(this, arguments);
            var e = new Event(type);
            e.stateInfo = __assign({}, arguments);
...
var HistoryRouter = (function () {
    function HistoryRouter() {
        this.routeTable = new rutelist();
    }
    HistoryRouter.prototype.insertJS = function (url) {
        var _a;
        if (this.oldID) {
            (_a = document.querySelector("#".concat(this.oldID))) === null || _a === void 0 ? void 0 : _a.remove();
        }
        var newScript = document.createElement('script');
        var time = new Date().getTime();
        this.oldID = "js_".concat(time);
        newScript.id = this.oldID;
        newScript.src = "".concat(url, "?js=").concat(new Date().getTime());
        newScript.type = 'module';
        var body = document.querySelector('body');
        body === null || body === void 0 ? void 0 : body.insertAdjacentElement('beforeend', newScript);
    };
    HistoryRouter.prototype.Init = function () {
        var _this = this;
        addStateListener();
        window.addEventListener('pushState', function (evt) {
            control.historyStateChange(evt, _this.routeTable);
        }, false);
        window.addEventListener("hashchange", function (evt) {
            control.hashchange(evt, _this.routeTable);
        }, false);
        window.addEventListener("popstate", function (evt) {
            control.popstatechange(evt, _this.routeTable);
        }, false);
        window.addEventListener('load', function (evt) {
            control.loadchange(evt, _this.routeTable);
        }, false);
    };
    HistoryRouter.prototype.bind = function (path, fn) {
        this.routeTable.reg(path, fn);
    };
...
var tplHome = {
    index: "<div class=\"index row rowcolumn\"><div class=\"header row align-center text-left prow-16 pcol-10\"><div class=\"action-left flex1\"></div><div class=\"action-right\"><button type=\"primary\" class=\"vg-btn btn-brand\">\u4E3B\u9875</button>\n<button type=\"success\" class=\"vg-btn btn-brand\">\u9879\u76EE\u5730\u5740</button></div></div><div class=\"content flex1 overflow-y\"><ul class=\"list p-20 mt-20 row align-center flex-wrap\" fxtag=\"list\"></ul></div><div class=\"footer\"></div></div>",
};

var tplList = {
    tagHeader: "<button class=\"vg-btn btn-type-brand mr-10\">{{text}}</button>",
    tplHome: "<li class=\"list-item col-3 pcol-10 cursor-pointer\" data-url=\"{{path}}\" fxtag=\"list_item\"><div class=\"card mb-20 p-16 border-radius-md bg-gray-100\"><div class=\"cover border-radius-md\" style=\"background: url('{{coverUrl}}') no-repeat 0/100%;padding-bottom: 60%;\"></div><div class=\"title font-size-18 text-center mt-10\">{{ title }}</div></div>",
};

var pagesHome = function () {
    ...
    var oList = oApp.querySelector('[fxtag="list"]');
    template.nodeRenderFor(tplList.tplHome, data, oList);
    var listItem = document.querySelectorAll('[fxtag="list_item"]');
    var _loop_1 = function (i) {
        listItem[i].addEventListener('click', function (e) {
            history.pushState('', '', "".concat(listItem[i].dataset.url));
        });
    };
...
var tplExample = {
    index: "<div class='index row rowcolumn p-20'><div id=\"canvas\" class=\"flex1 bg-gray-100\"></div></div>",
    tag: "<div class='index row rowcolumn p-20'><div class=\"header pb-20 row align-center\" fxtag=\"header\"></div><div id=\"canvas\" class=\"flex1 bg-gray-100\"></div></div>",
};

var pagesRect = function () {
    var oApp = document.querySelector("#app");
    ...
    var width = 400;
    var height = 200;
    var x = clientWidth / 2 - width / 2;
    var y = clientHeight / 2 - height / 2;
...
var pagesLine = function () {
    ...
    var line = new Konva.Line({
        x: 400,
        points: [100, 100, 200, 300, 500, 400, 800, 500],
        stroke: "red",
        strokeWidth: 2,
        draggable: true,
        fill: "red",
...
var pagesRegularPolygon = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.index, {}, oApp);
    var el = document.getElementById("canvas");
    if (!el) {
...
var pagesArrow = function () {
    ...
    var layer = new Konva.Layer();
    stage.add(layer);
    var regularPolygon = new Konva.Arrow({
        x: clientWidth / 2,
        y: clientHeight / 2,
...
var pagesImage = function () {
    ...
    var layer = new Konva.Layer();
    stage.add(layer);
    Konva.Image.fromURL('http://192.168.5.235:8060/src/assets/address.png', function (image) {
        image.setAttrs({
            x: clientWidth / 2 - 25,
            y: clientHeight / 2 - 25,
            height: 50,
...
var pagesShowHide = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.tag, {}, oApp);
    var oHeader = oApp.querySelector('[fxtag="header"]');
    ...
    layer.add(rect);
    var aBtn = oHeader.querySelectorAll('button');
    aBtn[0].addEventListener('click', function () {
        if (!stage) {
            return;
        }
        var shapes = stage.findOne("Rect");
        shapes === null || shapes === void 0 ? void 0 : shapes.show();
    });
    aBtn[1].addEventListener('click', function () {
        ...
        var shapes = stage.findOne("Rect");
        shapes === null || shapes === void 0 ? void 0 : shapes.hide();
    });
};

var pagesKeyEvents = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.tag, {}, oApp);
    var oHeader = oApp.querySelector('[fxtag="header"]');
    template.nodeRenderFor(tplList.tagHeader, [
...
var pagesGroup = function () {
    ...
    var colors = ["#ff8800", "#ff0000", "#ff00ff", "#00ffff"];
    for (var i = 0; i < 4; i++) {
        var rect = new Konva.Rect({
            id: "rect".concat(i),
            name: "testName",
    ...
    aBtn[0].addEventListener('click', function () {
        ...
        var shapes = stage.findOne("#rect0");
        shapes === null || shapes === void 0 ? void 0 : shapes.moveToTop();
    });
    aBtn[1].addEventListener('click', function () {
        if (!stage) {
            return;
        ...
        var shapes = stage.findOne("#rect0");
        shapes === null || shapes === void 0 ? void 0 : shapes.moveToBottom();
    });
    aBtn[2].addEventListener('click', function () {
        ...
        var shapes = stage.findOne("#rect0");
        shapes === null || shapes === void 0 ? void 0 : shapes.moveUp();
    });
    aBtn[3].addEventListener('click', function () {
        ...
        var shapes = stage.findOne("#rect0");
        shapes === null || shapes === void 0 ? void 0 : shapes.moveDown();
    });
};

var pagesTransformer = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.index, {}, oApp);
    var stage = null;
    var layer = new Konva.Layer();
    ...
    stage.on("mousedown touchstart", function (e) {
        ...
        var _a = stage.getPointerPosition(), x = _a.x, y = _a.y;
        x1 = x;
        x2 = x;
        y1 = y;
        y2 = y;
    ...
    stage.on("mousemove touchmove", function (e) {
        ...
        var _a = stage === null || stage === void 0 ? void 0 : stage.getPointerPosition(), x = _a.x, y = _a.y;
        x2 = x;
        y2 = y;
    ...
    stage.on("mouseup touchend", function (e) {
        ...
        var box = selectionRect.getClientRect();
...
var pagesPreview = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.index, {}, oApp);
    var windowDom = '<div id="preview" class="bg-gray-400 border-radius-sm" style="position:absolute;top:40px;right:40px;"></div>';
    var oCanvas = oApp.querySelector(".index");
    ...
    stage.on("mousedown touchstart", function (e) {
        ...
        e.evt.preventDefault();
        var _a = stage.getRelativePointerPosition(), x = _a.x, y = _a.y;
        x1 = x;
        x2 = x;
        y1 = y;
        y2 = y;
    ...
    stage.on("mousemove touchmove", function (e) {
        ...
        var _a = stage === null || stage === void 0 ? void 0 : stage.getRelativePointerPosition(), x = _a.x, y = _a.y;
        x2 = x;
        y2 = y;
        selectionRect.setAttrs({
            x: Math.min(x1, x2),
    ...
    function updatePreview() {
        ...
        layer.children.forEach(function (shape) {
            var clone = previewLayer.findOne("#".concat(shape.id()));
            if (!clone) {
                return;
            }
...
var shapesRect = (function () {
    function shapesRect(data) {
        this.x = data.x;
        this.init();
    }
    shapesRect.prototype.init = function () {
        this.newRect({});
    };
    shapesRect.prototype.newRect = function (data) {
        return new Konva.Rect(data);
    };
    return shapesRect;
}());

var pagesAdvanced = function () {
    var oApp = document.querySelector("#app");
    template.nodeRender(tplExample.index, {}, oApp);
    var el = document.getElementById("canvas");
    if (!el) {
...
var pages = function () {
    ...
    var _loop_1 = function (i) {
        router.bind(routerArr[i].path, function () { return __awaiter(void 0, void 0, void 0, function () {
