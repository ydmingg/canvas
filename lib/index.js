...
var aside = {
    index: "<div class=\"canvas-dom row rowcolumn p-20 gap-20\" fxtag=\"canvas_dom\"><div class=\"title row align-center\"><div class=\"row align-center flex1 gap-12\"><button class=\"vg-btn\" fxtag=\"button\">\u9ED8\u8BA4</button>\n<button class=\"vg-btn\" fxtag=\"view\">\u663E\u793A\u9690\u85CF</button></div><div class=\"row align-center width-auto gap-12\"><div>\u753B\u5E03\u5927\u5C0F\uFF1A</div><button class=\"vg-btn\" fxtag=\"size\"></button></div></div><div class=\"content flex1\" fxtag=\"content\"></div></div>",
};
...
var EventHandlers = (function () {
    ...
    EventHandlers.handleStageLoad = function (canvasRender) {
        var elementsBoundingBox = canvasRender.stage.getClientRect();
        var scaleBy = canvasRender.scale;
        if (elementsBoundingBox.width > canvasRender.stage.width() || elementsBoundingBox.height > canvasRender.stage.height()) {
            scaleBy = Math.min((canvasRender.stage.width() - 20) / elementsBoundingBox.width, (canvasRender.stage.height() - 20) / elementsBoundingBox.height);
        }
        var offset = {
            x: elementsBoundingBox.x,
            y: elementsBoundingBox.y
        };
        var newElementBoundingBoxWidth = Math.round((elementsBoundingBox.width + offset.x) * scaleBy);
        var newElementBoundingBoxHeight = Math.round((elementsBoundingBox.height + offset.y) * scaleBy);
    ...
    EventHandlers.handleWindowResize = function (canvasRender) {
        var newWidth = canvasRender.el.clientWidth;
        var newHeight = canvasRender.el.clientHeight;
    ...
    EventHandlers.handleWheel = function (event, canvasRender) {
        var position = canvasRender.stage.getPointerPosition();
        var step = 10;
        var direction = event.deltaY > 0 ? -1 : 1;
        var oldScale = canvasRender.stage.scaleX();
        var mousePointTo = {
    ...
    EventHandlers.handleKeyDown = function (event, canvasRender) {
        if (event.key === ' ') {
    ...
    EventHandlers.handleKeyUp = function (event, canvasRender) {
        if (event.key === ' ') {
            canvasRender.isSpacePressed = false;
            canvasRender.el.style.cursor = "default";
        }
    };
    EventHandlers.handleMouseDown = function (event, canvasRender) {
        if (event.button === 0 || event.button === 1) {
            canvasRender.handleStartDrag(event.clientX, event.clientY);
            canvasRender.el.style.cursor = "default";
        }
    };
    EventHandlers.handleMouseMove = function (event, canvasRender) {
        if ((event.buttons === 1 || event.buttons === 4) && canvasRender.isSpacePressed) {
            canvasRender.handleDrag(event.clientX, event.clientY);
        }
    };
    EventHandlers.handleMouseUp = function (canvasRender) {
        canvasRender.el.style.cursor = "default";
        canvasRender.handleEndDrag();
    };
    EventHandlers.handleStageMouseDown = function (event, canvasRender) {
        if (event.evt.button === 1) {
            canvasRender.handleStartDrag(event.evt.clientX, event.evt.clientY);
        }
    };
    EventHandlers.handleStageMouseMove = function (event, canvasRender) {
        if (event.evt.buttons === 4) {
            canvasRender.handleDrag(event.evt.clientX, event.evt.clientY);
            canvasRender.el.style.cursor = "grab";
        }
    };
    EventHandlers.handleStageMouseUp = function (canvasRender) {
        canvasRender.el.style.cursor = "default";
        canvasRender.handleEndDrag();
    };
    EventHandlers.handleCanvasClick = function (event, canvasRender) {
        var position = canvasRender.stage.getPointerPosition();
        var scale = canvasRender.stage.scaleX();
        ...
        if (canvasRender.isSpacePressed || !canvasRender.isCommentClickBool)
            return;
        canvasRender.isCanvasClickFlag = true;
    };
    EventHandlers.handleTouchStart = function (event, canvasRender) {
    ...
    EventHandlers.handleTouchMove = function (event, canvasRender) {
        ...
        if (event.touches.length === 1) {
            ...
            if (canvasRender.pinchStartDistance !== null) {
                ...
                var scale = currentDistance / canvasRender.pinchStartDistance;
                var stage = canvasRender.stage;
                var oldScale = stage.scaleX();
                var newScale = oldScale * scale;
                var offsetX = stage.x() - midpoint.x;
                var offsetY = stage.y() - midpoint.y;
                ...
                var initialDistance = Math.hypot(event.touches[0].clientX - event.touches[1].clientX, event.touches[0].clientY - event.touches[1].clientY);
                canvasRender.pinchStartDistance = initialDistance;
            }
        }
    };
    EventHandlers.handleTouchEnd = function (event, canvasRender) {
        canvasRender.handleEndDrag();
        canvasRender.pinchStartDistance = null;
        canvasRender.isTouchMovePosition = false;
    };
    EventHandlers.handleTap = function (event, canvasRender) {
        if (!canvasRender.isSpacePressed)
            return;
        var position = canvasRender.stage.getPointerPosition();
        var scale = canvasRender.stage.scaleX();
        canvasRender.x = Math.round((position.x - canvasRender.stage.x()) / scale);
...
var OtherMethods = (function () {
    function OtherMethods() {
    }
    OtherMethods.handleStartDrag = function (startX, startY, canvasRender) {
        canvasRender.lastDragPosition = { x: startX, y: startY };
        canvasRender.lastStagePosition = {
            x: canvasRender.stage.x(),
            y: canvasRender.stage.y()
        };
    };
    OtherMethods.handleDrag = function (mouseX, mouseY, canvasRender) {
        if (!canvasRender.lastDragPosition || !canvasRender.lastStagePosition)
            return;
        var deltaX = mouseX - canvasRender.lastDragPosition.x;
        var deltaY = mouseY - canvasRender.lastDragPosition.y;
        canvasRender.stage.x(canvasRender.lastStagePosition.x + deltaX);
        canvasRender.stage.y(canvasRender.lastStagePosition.y + deltaY);
        canvasRender.stage.batchDraw();
    };
    OtherMethods.handleEndDrag = function (canvasRender) {
        canvasRender.lastDragPosition = null;
        canvasRender.lastStagePosition = null;
    };
    OtherMethods.handleCommentSize = function (canvasRender) {
        var commentsNode = canvasRender.spaceArr.filter(function (item) { return item.type === 'Component_Comment'; });
        commentsNode.forEach(function (e) {
            var point = e.shape[0];
            var commentsStartNumber = 1;
            point.scaleX(commentsStartNumber / canvasRender.scale);
            point.scaleY(point.scaleX());
        });
    };
    OtherMethods.handlePinchZoom = function (scale, canvasRender) {
        var newScale = canvasRender.stage.scaleX() * scale;
        var minScale = 0.02;
        var maxScale = 256;
        var clampedScale = Math.min(maxScale, Math.max(minScale, newScale));
    ...
    OtherMethods.elementsVisible = function (elementTypeName, isCommentVisible, canvasRender) {
        var commentsNode = canvasRender.spaceArr.filter(function (item) { return item.type === elementTypeName; });
        commentsNode.forEach(function (el) {
            var point = el.shape[0];
...
var newComment = (function () {
    function newComment(params, cRender) {
...
var newView = (function () {
    function newView(params, canvasRender) {
        var _this = this;
        this.imgElement = new Image();
        this.imgElement.onload = function () {
            var defaultWidth = params.width || _this.imgElement.width;
            var defaultHeight = params.height || _this.imgElement.height;
            _this.image.width(defaultWidth);
            _this.image.height(defaultHeight);
            _this.image.image(_this.imgElement);
            canvasRender.stage.draw();
...
var CanvasRender = (function () {
    function CanvasRender(el, width, height) {
        var _this = this;
        this.x = 0;
        this.y = 0;
        this.scale = 1;
        ...
        this.isTouchMovePosition = false;
        this.isCommentClickBool = true;
        this.isCanvasClickFlag = false;
        this.isCommentVisible = true;
        this.pinchStartDistance = null;
        this.lastDragPosition = null;
        this.lastStagePosition = null;
        this.render = function (Data) {
            Data.forEach(function (item) {
                var type = item.type, params = item.params;
                _this["Canvas".concat(type)](params);
            });
            _this.stage.draw();
        };
        this.el = el;
        ...
        this.commentGroup = new Konva.Group();
    ...
    CanvasRender.prototype.init = function () {
        ...
        this.layer.add(this.commentGroup);
        window.addEventListener('resize', function () { return EventHandlers.handleWindowResize(_this); });
        this.el.addEventListener('wheel', function (event) { return EventHandlers.handleWheel(event, _this); });
        window.addEventListener('keydown', function (event) { return EventHandlers.handleKeyDown(event, _this); });
        window.addEventListener('keyup', function (event) { return EventHandlers.handleKeyUp(event, _this); });
        this.el.addEventListener('mousedown', function (event) { return EventHandlers.handleMouseDown(event, _this); });
        this.el.addEventListener('mousemove', function (event) { return EventHandlers.handleMouseMove(event, _this); });
        this.el.addEventListener('mouseup', function () { return EventHandlers.handleMouseUp(_this); });
        this.stage.on('mousedown', function (event) { return EventHandlers.handleStageMouseDown(event, _this); });
        this.stage.on('mousemove', function (event) { return EventHandlers.handleStageMouseMove(event, _this); });
        this.stage.on('mouseup', function () { return EventHandlers.handleStageMouseUp(_this); });
        this.stage.on('click tap', function (event) { return EventHandlers.handleCanvasClick(event, _this); });
        this.el.addEventListener('touchstart', function (event) { return EventHandlers.handleTouchStart(event, _this); });
        this.el.addEventListener('touchmove', function (event) { return EventHandlers.handleTouchMove(event, _this); });
        this.el.addEventListener('touchend', function (event) { return EventHandlers.handleTouchEnd(event, _this); });
        this.el.addEventListener('click', function (event) { return EventHandlers.handleTap(event, _this); });
    };
    CanvasRender.prototype.CanvasComponent_Comment = function (params) {
        if (this.isSpacePressed || !this.isCommentClickBool || this.isTouchMovePosition)
            return;
        var container = new newComment(params, this);
        this.commentGroup.add(container.group);
        this.commentGroup.moveToTop();
        ...
        container.group.x(adjustedX);
        container.group.y(adjustedY);
        this.spaceArr.push({
            id: params.id,
            type: 'Component_Comment',
            shape: [container.group]
        });
        this.handleCommentSize();
    };
    CanvasRender.prototype.CanvasComponent_View = function (params) {
        var view = new newView(params, this);
    ...
    CanvasRender.prototype.handleStartDrag = function (startX, startY) { OtherMethods.handleStartDrag(startX, startY, this); };
    CanvasRender.prototype.handleDrag = function (mouseX, mouseY) { OtherMethods.handleDrag(mouseX, mouseY, this); };
    CanvasRender.prototype.handleEndDrag = function () { OtherMethods.handleEndDrag(this); };
    CanvasRender.prototype.handleCommentSize = function () { OtherMethods.handleCommentSize(this); };
    CanvasRender.prototype.handlePinchZoom = function (scale) { OtherMethods.handlePinchZoom(scale, this); };
    CanvasRender.prototype.elementsVisible = function (elementTypeName, isCommentVisible) { OtherMethods.elementsVisible(elementTypeName, isCommentVisible, this); };
    return CanvasRender;
}());

var init$1 = function (canvas) {
    var src = "https://static.funxdata.com/2023/12/28/3d482287a2bf4f4c9d5ec19a4ff7449d.png";
    var data = [
        {
            type: "Component_View",
            params: {
...
var init = function (canvas) {
    var viewBtn = document.querySelector('[fxtag="view"]');
    var sizeBtn = document.querySelector('[fxtag="size"]');
    var bool = false;
...
var el = document.querySelector('[fxtag="content"]');
var clientWidth = el.clientWidth, clientHeight = el.clientHeight;
var canvas = new CanvasRender(el, clientWidth, clientHeight);
init$1(canvas);
init(canvas);
