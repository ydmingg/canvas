...
function handleStartDrag(startX: number, startY: number, canvasRender: CanvasRender) {
...
function handleDrag(mouseX: number, mouseY: number, canvasRender: CanvasRender) { 
    // 如果上次拖拽的位置信息不存在，则直接返回（意味着还没有开始拖拽或者已经清理了状态）。
    if(!canvasRender.lastDragPosition || !canvasRender.lastStagePosition) return
    ...
    // 刷新整个舞台以应用新的变换并重绘所有元素。
    canvasRender.stage.batchDraw();
}

// 处理在拖拽操作结束时被调用以重置相关状态。
function handleEndDrag(canvasRender: CanvasRender) { 
